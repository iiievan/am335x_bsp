...
namespace
{
    ...
    PingPongContext g_pingpong_ctx;
...
bool test_qdma_channel_chain(const uint8_t qch)
{
    ...
    const auto [param_first, param_last] = PaRAMFactory::makeQdmaLink(reinterpret_cast<uintptr_t>(src_buf),
                                                                                                      reinterpret_cast<uintptr_t>(dst_buf),
                                                                                                      HALF,
                                                                                                      static_cast<uint8_t>(param1),
                                                                                                      tcc);
    if (param_first.OPT.b.TCCHEN != 0u ||
        param_first.OPT.b.TCINTEN != 0u ||
        param_first.OPT.b.STATIC != 0u ||
        param_last.OPT.b.TCCHEN != 0u ||
