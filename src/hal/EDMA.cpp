 ...
 */
void init(const e_EVENT_QUEUE que_num)
{     
    ...
    cc.CCERRCLR.reg =   SET_ALL_BITS;        // Clear CCERR register
    ...
    cc.QRAE[static_cast<uint8_t>(region_id)].reg =  SET_ALL_BITS;
 ...
 *
 */
void enable_ch_in_shadow_reg(const e_EDMA3_CH_TYPE ch_type,const uint32_t ch_num)
{     
    auto& cc = *AM335X_EDMA3CC;
 ...
 *
 */
void disable_ch_in_shadow_reg(const e_EDMA3_CH_TYPE ch_type,const uint32_t ch_num)
{     
    auto& cc = *AM335X_EDMA3CC;
 ...
 *
 */
void map_ch_to_evtQ(const e_EDMA3_CH_TYPE ch_type, const uint32_t ch_num,  const e_EVENT_QUEUE evt_Qnum)
{
    auto& cc = *AM335X_EDMA3CC;
 ...
 *
 */
void unmap_ch_to_evtQ(const e_EDMA3_CH_TYPE ch_type, const uint32_t ch_num)
{
    auto& cc = *AM335X_EDMA3CC;
 ...
 *
 */
void map_QDMA_ch_to_paRAM(const uint32_t ch_num, uint32_t *paRAM_id)
{
    auto& cc = *AM335X_EDMA3CC;
    ...
    if (( AM335X_DMACH_MAX + ch_num) == (*paRAM_id))
    {
        ...
        cc.QCHMAP[ch_num].b.PAENTRY = 0;
        cc.QCHMAP[ch_num].b.PAENTRY = (*paRAM_id);
    }
    else
    {
 ...
 *
 */
void set_QDMA_trig_word(const uint32_t ch_num, const uint8_t trig_word)
{
    auto& cc = *AM335X_EDMA3CC;
    
     // Set the Trigger Word
     //HWREG(baseAdd + EDMA3CC_QCHMAP(ch_num)) |= EDMA3CC_QCHMAP_TRWORD_SET(trigWord);
     cc.QCHMAP[ch_num].b.TRWORD = trig_word;
}
 ...
 *
 */
void clr_miss_evt(const uint32_t ch_num)
{     
    auto& cc = *AM335X_EDMA3CC;
    
    if(ch_num < 32)
    {
         //HWREG(baseAdd + EDMA3CC_SECR(region_id)) = (0x01u << ch_num);    // clear SECR to clean any previous NULL request
         //HWREG(baseAdd + EDMA3CC_EMCR) |= (0x01u <<  ch_num);               //clear EMCR to clean any previous NULL request
          get_SECR_ptr(region_id)->reg = (0x01u << ch_num);    // clear SECR to clean any previous NULL request
         cc.EMCR.reg |= (0x01u <<  ch_num);               //clear EMCR to clean any previous NULL request
    }
    else
    {
         //HWREG(baseAdd + EDMA3CC_SECRH(region_id)) = (0x01u << (ch_num - 32));
         //HWREG(baseAdd + EDMA3CC_EMCRH) |= (0x01u <<  (ch_num - 32));         //clear EMCRH to clean any previous NULL request
          get_SECR_ptr(region_id)->reg = (0x01u << (ch_num - 32));
 ...
 *
 */
void QDMA_clr_miss_evt(const uint32_t ch_num)
{    
    auto& cc = *AM335X_EDMA3CC;
    
    // clear SECR to clean any previous NULL request
    //HWREG(baseAdd + EDMA3CC_QSECR(region_id)) = (0x01u << ch_num);
     get_QSECR_ptr(region_id)->reg = (0x01u << ch_num);
 ...
 *
 */
void clr_CCERR(const uint32_t flags)
{
    auto& cc = *AM335X_EDMA3CC;
 ...
 */
void set_evt(const uint32_t ch_num)
{ 
    if(ch_num < 32)
    {
         // (ESR) - set corresponding bit to set a event
         //HWREG(baseAdd + EDMA3CC_ESR(region_id)) |= (0x01u <<  ch_num);
          get_ESR_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
    else
    {
         // (ESRH) - set corresponding bit to set a event
         //HWREG(baseAdd + EDMA3CC_ESRH(region_id)) |= (0x01u << (ch_num - 32));
          get_ESRH_ptr(region_id)->reg |= (0x01u << (ch_num - 32));
    }
}
 ...
 *  Note :   This API is generally used during Manual transfers.\n
 */
void clr_evt(const uint32_t ch_num)
{
    if(ch_num < 32)
    {
         // (ECR) - set corresponding bit to clear a event
         //HWREG(baseAdd + EDMA3CC_ECR(region_id)) |= (0x01u <<  ch_num);
          get_ECR_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
    else
    {
         // (ECRH) - set corresponding bit to clear a event
         //HWREG(baseAdd + EDMA3CC_ECRH(region_id)) |= (0x01u << (ch_num - 32));
          get_ECRH_ptr(region_id)->reg |= (0x01u << (ch_num - 32));
    }
}
 ...
 */
void enable_DMA_evt(const uint32_t ch_num)
{
    if(ch_num < 32)
    {
         // (EESR) - set corresponding bit to enable DMA event
         //HWREG(baseAdd + EDMA3CC_EESR(region_id)) |= (0x01u <<  ch_num);
         get_EESR_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
    else
    {
         // (EESRH) - set corresponding bit to enable DMA event
         //HWREG(baseAdd + EDMA3CC_EESRH(region_id)) |= (0x01u << (ch_num - 32));
         get_EESRH_ptr(region_id)->reg |= (0x01u << (ch_num - 32));
    }
}
 ...
 */
void disable_DMA_evt(const uint32_t ch_num)
{    
    if(ch_num < 32)
    {
         // (EECR) - set corresponding bit to disable event
         //HWREG(baseAdd + EDMA3CC_EECR(region_id)) |= (0x01u <<  ch_num);
         get_EECR_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
    else
    {
         // (EECRH) - set corresponding bit to disable event
         //HWREG(baseAdd + EDMA3CC_EECRH(region_id)) |= (0x01u <<  ch_num);
         get_EECRH_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
}
 ...
 *  Note :   Writes of 1 to the bits in QEESR sets the corresponding event
              bits in QEER.\n
 */
void enable_QDMA_evt(const uint32_t ch_num)
{
    // (QEESR) - set corresponding bit to enable QDMA event
    //HWREG(baseAdd + EDMA3CC_QEESR(region_id)) = (0x01u << ch_num);
     get_QEESR_ptr(region_id)->reg = (0x01u << ch_num);
}
 ...
 *  Note :   Writes of 1 to the bits in QEECR clears the corresponding event
              bits in QEER.\n
 */
void disable_QDMA_evt(const uint32_t ch_num)
{
     
    
    // (QEESR) - set corresponding bit to enable QDMA event
    //HWREG(baseAdd + EDMA3CC_QEECR(region_id)) = (0x01u << ch_num);
     get_QEECR_ptr(region_id)->reg = (0x01u << ch_num);
}
...
{    
    ...
    //intr_sts = (uint32_t)HWREG(baseAdd + EDMA3CC_IPR(region_id));
    intr_sts = static_cast<uint32_t>(get_IPR_ptr(region_id)->reg);
 ...
 */
void enable_evt_intr(const uint32_t ch_num)
{
    if(ch_num < 32)
    {
         //  Interrupt Enable Set Register (IESR)
         // HWREG(baseAdd + EDMA3CC_IESR(region_id)) |= (0x01u <<  ch_num);
          get_IESR_ptr(region_id)->reg |= (0x01u <<  ch_num);;
    }
    else
    {
         //  Interrupt Enable Set Register (IESRH)
         // HWREG(baseAdd + EDMA3CC_IESRH(region_id)) |= (0x01u << (ch_num - 32));
          get_IESRH_ptr(region_id)->reg |= (0x01u << (ch_num - 32));
    }
}
 ...
 *           no effect.\n
 */
void disable_evt_intr(const uint32_t ch_num)
{
    ...
    if(ch_num < 32)
    {
         ...
         // HWREG(baseAdd + EDMA3CC_IECR(region_id)) |= (0x01u <<  ch_num);
         get_IECR_ptr(region_id)->reg |= (0x01u <<  ch_num);
    }
    else
    {
         // Interrupt Enable Clear Register (IECRH)
         // HWREG(baseAdd + EDMA3CC_IECRH(region_id)) |= (0x01u << (ch_num - 32));
         get_IECRH_ptr(region_id)->reg |= (0x01u << (ch_num - 32));
    }
}
 ...
 *
 */
void clr_intr(const uint32_t value)
{     
    if(value < 32)
    {
         //HWREG(baseAdd + EDMA3CC_ICR(region_id)) = (1u << value);
         get_ICR_ptr(region_id)->reg = (1u << value);
    }
    else
    {
         //HWREG(baseAdd + EDMA3CC_ICRH(region_id)) = (1u << (value - 32));
          get_ICRH_ptr(region_id)->reg = (1u << (value - 32));
    }
}
 ...
 */
 paRAM_entry_t* get_paRAM(uint32_t paRAM_id)
{
    auto *sr =  get_paRAM_ptr(paRAM_id);
 ...
 * @param   paRAM_id              User gets the existing PaRAM here.\n
 */
 paRAM_entry_t* QDMA_get_paRAM(const uint32_t paRAM_id)
{
    const auto sr =  get_paRAM_ptr(paRAM_id);
    return reinterpret_cast< paRAM_entry_t*>(sr);
}
 ...
 *                                 PaRAM.\n
 */
void set_paRAM(const uint32_t ch_num, const paRAM_entry_t* new_paRAM)
{
 ...
 *                                 PaRAM.\n
 */
void QDMA_set_paRAM(uint32_t paRAM_id,  paRAM_entry_t* new_paRAM)
{
 ...
 */
void QDMA_set_paRAM_entry(const uint32_t paRAM_id, const uint32_t paRAM_entry, const uint32_t new_paRAM_entry_val)
{
 ...
 */
uint32_t QDMA_get_paRAM_entry(const uint32_t paRAM_id, const uint32_t paRAM_entry)
{
 ...
 */
bool  request_channel(const e_EDMA3_CH_TYPE ch_type, const uint32_t ch_num, const uint32_t tcc_num,  const e_EVENT_QUEUE evt_Qnum)
{
    bool ret_val = false;
    ...
    if (ch_num <  AM335X_DMACH_MAX)
    {
        ...
        if ( CHANNEL_TYPE_DMA == ch_type)
        {
            ...
            get_paRAM(ch_num)->OPT.b.TCC = 0x0;
 ...
 */
bool  free_channel(const e_EDMA3_CH_TYPE ch_type, const uint32_t ch_num, const uint32_t trig_mode, const uint32_t tcc_num, const e_EVENT_QUEUE evt_Qnum)
{
    bool ret_val = false;
    (void)evt_Qnum;
 ...
 *
 */
bool  enable_transfer(const uint32_t ch_num, const uint32_t trig_mode)
{
    bool ret_val = false;
 ...
 *
 */
bool  disable_transfer(const uint32_t ch_num, const uint32_t trig_mode)
{
    bool ret_val = false;
 ...
 *  @return none.\n
 */
void clear_error_bits(const uint32_t ch_num, const e_EVENT_QUEUE evt_Qnum)
{     
    auto& cc = *AM335X_EDMA3CC;
    ...
    if(ch_num <  AM335X_DMACH_MAX)
    {
         if(ch_num < 32)
         {
              ...
              //HWREG(baseAdd + EDMA3CC_SECR(region_id)) = (0x01u << ch_num);   // Clears the SER
              get_EECR_ptr(region_id)->reg = (0x01u << ch_num);
              cc.EMCR.reg = (0x01u << ch_num);                  // Write to EMCR to clear the corresponding EMR bit
              get_SECR_ptr(region_id)->reg = (0x01u << ch_num);     // Clears the SER

         }
         else
         {
              //HWREG(baseAdd + EDMA3CC_EECRH(region_id)) = (0x01u << (ch_num - 32));
              //HWREG(baseAdd + EDMA3CC_EMCRH) = (0x01u << (ch_num - 32));        // Write to EMCR to clear the corresponding EMR bi
              //HWREG(baseAdd + EDMA3CC_SECRH(region_id)) = (0x01u << (ch_num - 32));   // Clears the SER
              get_EECRH_ptr(region_id)->reg = (0x01u << (ch_num - 32));
              cc.EMCRH.reg = (0x01u << (ch_num - 32));                // Write to EMCRH to clear the corresponding EMR bi
              get_SECRH_ptr(region_id)->reg = (0x01u << (ch_num - 32));   // Clears the SER
         }
    ...
    */
    ...
    {
        case  EVENT_Q0:
          cc.CCERRCLR.b.QTHRXCD0 = 0x0;
          cc.CCERRCLR.b.TCCERR = 0x0;
          break;
        case  EVENT_Q1:
          cc.CCERRCLR.b.QTHRXCD1 = 0x0;
          cc.CCERRCLR.b.TCCERR = 0x0;
          break;
        case  EVENT_Q2:
          cc.CCERRCLR.b.QTHRXCD2 = 0x0;
          cc.CCERRCLR.b.TCCERR = 0x0;
          break;
        default:
          break;
 ...
 *
 */
 QEMR_reg_t QDMA_get_ERR_intr_status()
{
    auto& cc = *AM335X_EDMA3CC;
    ...
    intr_status_val.reg = cc.QEMR.reg;
 ...
 */
void deinit(const uint32_t que_num)
{     
    auto& cc = *AM335X_EDMA3CC;
    (void)que_num;
    ...
    //HWREG(baseAdd + EDMA3CC_EMCR)  = EDMA3_SET_ALL_BITS;
    //HWREG(baseAdd + EDMA3CC_EMCRH) = EDMA3_SET_ALL_BITS;
    cc.EMCR.reg =  CLR_ALL_BITS;
    cc.EMCRH.reg =  CLR_ALL_BITS;
...
{    
    ...
    //intr_sts_val = (uint32_t)HWREG(baseAdd + EDMA3CC_IPRH(region_id));
    intr_sts_val = get_IPRH_ptr(region_id)->reg;
 ...
 */
void channel_to_param_map(const uint32_t channel, const uint32_t param_set)
{
    auto& cc = *AM335X_EDMA3CC;
    //HWREG(baseAdd + EDMA3CC_DCHMAP(channel)) = param_set << 5;
    cc.DCHMAP[channel].b.PAENTRY = param_set;
}
/**
...
void context_save(EDMACONTEXT_t *p_edma_cntx)
{
    ...
    for(i = 0; i <  AM335X_DMACH_MAX; i++)
    {
         ...
         p_edma_cntx->dch_map[i] = cc.DCHMAP[i].b.PAENTRY;
    }
    ...
    //p_edma_cntx->event_set_reg_low  = HWREG(baseAddr + EDMA3CC_ESR(0));
    //p_edma_cntx->event_set_reg_high = HWREG(baseAddr + EDMA3CC_ESRH(0));
    p_edma_cntx->event_set_reg_low  =  get_ESR_ptr(region_id)->reg;
    p_edma_cntx->event_set_reg_high =  get_ESRH_ptr(region_id)->reg;

    // Get Event Enable Set Register value
    //p_edma_cntx->enable_evt_set_reg_low = HWREG(baseAddr + EDMA3CC_EER(0));
    //p_edma_cntx->enable_evt_set_reg_high = HWREG(baseAddr + EDMA3CC_EERH(0));
    p_edma_cntx->enable_evt_set_reg_low =   get_EER_ptr(region_id)->reg;
    p_edma_cntx->enable_evt_set_reg_high =  get_EERH_ptr(region_id)->reg;

    // Get Interrupt Enable Set Register value
    p_edma_cntx->int_enable_set_reg_low  =   get_IER_ptr(region_id)->reg;
    p_edma_cntx->int_enable_set_reg_high =   get_IERH_ptr(region_id)->reg;
...
{     
    ...
    for(i = 0; i <  AM335X_DMACH_MAX; i++)
    {
         ...
         cc.DCHMAP[i].b.PAENTRY = p_edma_cntx->dch_map[i];
    }
    ...
    //HWREG(baseAddr + EDMA3CC_ESR(0)) = p_edma_cntx->event_set_reg_low;
    //HWREG(baseAddr + EDMA3CC_ESRH(0)) = p_edma_cntx->event_set_reg_high;
     get_ESR_ptr(region_id)->reg = p_edma_cntx->event_set_reg_low;
     get_ESRH_ptr(region_id)->reg = p_edma_cntx->event_set_reg_high;

    // set Event Enable Set Register value
    //HWREG(baseAddr + EDMA3CC_EESR(0)) = p_edma_cntx->enable_evt_set_reg_low;
    //HWREG(baseAddr + EDMA3CC_EESRH(0)) = p_edma_cntx->enable_evt_set_reg_high;
     get_EER_ptr(region_id)->reg = p_edma_cntx->enable_evt_set_reg_low;
     get_EERH_ptr(region_id)->reg = p_edma_cntx->enable_evt_set_reg_high;

    // set Interrupt Enable Set Register value
    //HWREG(baseAddr + EDMA3CC_IESR(0)) = p_edma_cntx->int_enable_set_reg_low;
    //HWREG(baseAddr + EDMA3CC_IESRH(0)) = p_edma_cntx->int_enable_set_reg_high;
     get_IER_ptr(region_id)->reg = p_edma_cntx->int_enable_set_reg_low;
     get_IERH_ptr(region_id)->reg = p_edma_cntx->int_enable_set_reg_high;
