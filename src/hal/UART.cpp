...
namespace HAL::UART
{
    ...
    * 'mode' can take one of the following values:
    ...
    void uart_base::FIFO_register_write(REGS::UART::FCR_reg_t fcr) noexcept
    {
        ...
        // FCR is write-only. Keep the complete persistent configuration so
        // later FIFO clear operations cannot silently reset trigger fields.
        m_FCR_shadow = fcr;
        m_FCR_shadow.b.RX_FIFO_CLEAR = 0u;
        m_FCR_shadow.b.TX_FIFO_CLEAR = 0u;

        if(divisor_before.raw != 0x0000)
            divisor_latch_set(divisor_before);
    }

    bool uart_base::FIFO_configure(const uint8_t tx_trigger_level,
                                   const uint8_t rx_trigger_level,
                                   const REGS::UART::e_SCR_DMA_MODE dma_mode) noexcept
    {
        using namespace REGS::UART;

        if (tx_trigger_level == 0u || tx_trigger_level > 63u ||
            rx_trigger_level == 0u || rx_trigger_level > 63u ||
            static_cast<uint32_t>(dma_mode) > static_cast<uint32_t>(SCR_DMA_MODE_3))
        {
            return false;
        }

        LCR_reg_t saved_lcr{};
        saved_lcr.reg = m_instance.LCR.reg;

        switch_reg_config_mode(CONFIG_MODE_B, ENH_DISABLE);
        const bool saved_enhanced = m_instance.EFR.b.ENHANCEDEN != 0u;
        m_instance.EFR.b.ENHANCEDEN = 1u;

        switch_reg_config_mode(CONFIG_MODE_A, ENH_DISABLE);
        const bool saved_tcr_tlr = m_instance.MCR.b.TCRTLR != 0u;
        m_instance.MCR.b.TCRTLR = 1u;

        m_instance.SCR.b.TXTRIGGRANU1 = 1u;
        m_instance.SCR.b.RXTRIGGRANU1 = 1u;
        m_instance.SCR.b.DMAMODECTL = 1u;
        m_instance.SCR.b.DMAMODE2 = dma_mode;

        m_instance.TLR.b.TX_FIFO_TRIG_DMA = (tx_trigger_level >> 2u) & 0x0Fu;
        m_instance.TLR.b.RX_FIFO_TRIG_DMA = (rx_trigger_level >> 2u) & 0x0Fu;

        FCR_reg_t fcr{};
        fcr.b.FIFO_EN = 1u;
        fcr.b.RX_FIFO_CLEAR = 1u;
        fcr.b.TX_FIFO_CLEAR = 1u;
        fcr.b.TX_FIFO_TRIG = tx_trigger_level & 0x03u;
        fcr.b.RX_FIFO_TRIG = rx_trigger_level & 0x03u;
        FIFO_register_write(fcr);

        // MCR[6] is writable only while enhanced access is enabled.
        switch_reg_config_mode(CONFIG_MODE_B, ENH_DISABLE);
        m_instance.EFR.b.ENHANCEDEN = 1u;
        switch_reg_config_mode(CONFIG_MODE_A, ENH_DISABLE);
        m_instance.MCR.b.TCRTLR = saved_tcr_tlr ? 1u : 0u;
        switch_reg_config_mode(CONFIG_MODE_B, ENH_DISABLE);
        m_instance.EFR.b.ENHANCEDEN = saved_enhanced ? 1u : 0u;
        m_instance.LCR.reg = saved_lcr.reg;
        return true;
    }

    void uart_base::FIFO_clear(const bool clear_tx, const bool clear_rx) noexcept
    {
        REGS::UART::FCR_reg_t fcr = m_FCR_shadow;
        fcr.b.TX_FIFO_CLEAR = clear_tx ? 1u : 0u;
        fcr.b.RX_FIFO_CLEAR = clear_rx ? 1u : 0u;
        FIFO_register_write(fcr);
    }
    ...
    void uart_base::DMA_disable() noexcept
    {
        DMA_enable(REGS::UART::SCR_DMA_MODE_0);
    }

    bool uart_base::TX_DMA_threshold_configure(const uint8_t threshold) noexcept
    {
        if (threshold > 63u)
            return false;

        m_instance.MDR3.b.SET_DMA_TX_THRESHOLD = 1u;
        m_instance.TX_DMA_THRESHOLD.b.TX_DMA_THRESHOLD = threshold;
        return true;
    }
